...
def extract_data():
    ...
    df.to_csv('/opt/airflow/dvc_repo/dataset.csv', index=False)

def transform_data():
    news_links_dataset_v1 = pd.read_csv('/opt/airflow/dvc_repo/dataset.csv')
    ...
    news_links_dataset_v3.to_csv('/opt/airflow/dvc_repo/dataset2.csv', index=False)
...
dvc_remote_add_task = BashOperator(
    ...
    bash_command='cd /opt/airflow/dvc_repo && dvc remote add -d my_google_drive gdrive://1_QxerY34BmGEccndTUFJH8fXGNWJb5_S -f',
    dag=dag,
)
...
dvc_add_command = f"cd /opt/airflow/dvc_repo && pwd && dvc add dataset2.csv"
